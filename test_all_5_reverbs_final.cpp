// Final comprehensive test suite for all 5 professional reverb implementations
#include <iostream>
#include <memory>
#include <cmath>
#include <iomanip>
...
public:
    ...
    void testReverb(EngineBase* reverb, const std::string& name) {
        ...
        // Test 6: Special Features (if applicable)
        testSpecialFeatures(reverb, name);
    }
    ...
    void testReverbTail(EngineBase* reverb, const std::string& name) {
        ...
        if (name == "GatedReverb") {
            ...
            params[1] = 0.0f; // Concert Hall (longest IR)
            params[2] = 1.0f; // Full size
        } else {
    ...
    void testSpecialFeatures(EngineBase* reverb, const std::string& name) {
        std::cout << "\n--- Test 6: Special Features ---" << std::endl;
        
        if (name == "ConvolutionReverb") {
            // Test IR selection and reversal
            std::cout << "  Testing IR selection and reversal..." << std::endl;
            
            reverb->reset();
            std::map<int, float> params;
            params[0] = 1.0f; // Full wet
            params[1] = 0.0f; // Concert Hall
            params[5] = 0.0f; // Not reversed
            reverb->updateParameters(params);
            ...
            reverb->process(buffer1);
            float normalEnergy = buffer1.getRMSLevel(0, 0, 512);
            ...
            reverb->process(buffer2);
            float reversedEnergy = buffer2.getRMSLevel(0, 0, 512);
            
            bool hasSpecialFeature = std::abs(normalEnergy - reversedEnergy) > 0.001f;
            results.push_back({name + " Special Feature", hasSpecialFeature, 
                "IR Reversal " + std::string(hasSpecialFeature ? "working" : "not working")});
            
            std::cout << "  Normal: " << normalEnergy << ", Reversed: " << reversedEnergy << std::endl;
            std::cout << (hasSpecialFeature ? "  ✓ PASS" : "  ✗ FAIL") << std::endl;
            
        } else if (name == "ShimmerReverb") {
            // Test pitch shifting
            std::cout << "  Testing shimmer pitch effect..." << std::endl;
            
            reverb->reset();
            std::map<int, float> params;
            params[0] = 1.0f; // Full wet
            params[1] = 0.0f; // No pitch shift
            reverb->updateParameters(params);
            
            juce::AudioBuffer<float> buffer1(2, 512);
            for (int i = 0; i < 512; i++) {
                float val = std::sin(2.0f * M_PI * 440.0f * i / 44100.0f) * 0.5f;
                buffer1.setSample(0, i, val);
                buffer1.setSample(1, i, val);
            }
            reverb->process(buffer1);
            float noPitchRMS = buffer1.getRMSLevel(0, 0, 512);
            
            // Test with pitch shift
            reverb->reset();
            params[1] = 1.0f; // Octave up
            params[2] = 1.0f; // Full shimmer
            reverb->updateParameters(params);
            
            juce::AudioBuffer<float> buffer2(2, 512);
            for (int i = 0; i < 512; i++) {
                float val = std::sin(2.0f * M_PI * 440.0f * i / 44100.0f) * 0.5f;
                buffer2.setSample(0, i, val);
                buffer2.setSample(1, i, val);
            }
            reverb->process(buffer2);
            float withPitchRMS = buffer2.getRMSLevel(0, 0, 512);
            
            bool hasSpecialFeature = std::abs(noPitchRMS - withPitchRMS) > 0.001f;
            results.push_back({name + " Special Feature", hasSpecialFeature, 
                "Shimmer " + std::string(hasSpecialFeature ? "working" : "not working")});
            
            std::cout << "  No shimmer: " << noPitchRMS << ", With shimmer: " << withPitchRMS << std::endl;
            std::cout << (hasSpecialFeature ? "  ✓ PASS" : "  ✗ FAIL") << std::endl;
            
        } else if (name == "GatedReverb") {
            // Test gate threshold
            std::cout << "  Testing gate threshold response..." << std::endl;
            
            reverb->reset();
            std::map<int, float> params;
            params[0] = 1.0f; // Full wet
            params[1] = 0.9f; // High threshold
            reverb->updateParameters(params);
            
            // Low level signal (below threshold)
            juce::AudioBuffer<float> buffer1(2, 512);
            for (int i = 0; i < 512; i++) {
                buffer1.setSample(0, i, 0.01f);
                buffer1.setSample(1, i, 0.01f);
            }
            reverb->process(buffer1);
            float lowLevelRMS = buffer1.getRMSLevel(0, 0, 512);
            
            // High level signal (above threshold)
            reverb->reset();
            juce::AudioBuffer<float> buffer2(2, 512);
            for (int i = 0; i < 512; i++) {
                buffer2.setSample(0, i, 0.9f);
                buffer2.setSample(1, i, 0.9f);
            }
            reverb->process(buffer2);
            float highLevelRMS = buffer2.getRMSLevel(0, 0, 512);
            
            bool hasSpecialFeature = highLevelRMS > lowLevelRMS * 2.0f;
            results.push_back({name + " Special Feature", hasSpecialFeature, 
                "Gate " + std::string(hasSpecialFeature ? "working" : "not working")});
            
            std::cout << "  Below threshold: " << lowLevelRMS << ", Above threshold: " << highLevelRMS << std::endl;
            std::cout << (hasSpecialFeature ? "  ✓ PASS" : "  ✗ FAIL") << std::endl;
            
        } else {
            // Default test for other reverbs
            results.push_back({name + " Special Feature", true, "N/A"});
            std::cout << "  No special feature test for " << name << std::endl;
        }
    }
    
    void printSummary() {
        std::cout << "\n\n========================================" << std::endl;
        std::cout << "FINAL TEST SUMMARY - ALL 5 REVERBS" << std::endl;
        std::cout << "========================================" << std::endl;
        
        int passed = 0;
        int failed = 0;
        
        // Group results by reverb
        std::string currentReverb = "";
        for (const auto& result : results) {
            std::string reverbName = result.testName.substr(0, result.testName.find(' '));
            if (reverbName != currentReverb) {
                if (!currentReverb.empty()) std::cout << std::endl;
                std::cout << reverbName << ":" << std::endl;
                currentReverb = reverbName;
            }
            
            std::cout << "  " << (result.passed ? "✓" : "✗") << " " 
                      << result.testName.substr(result.testName.find(' ') + 1)
        ...
        if (failed == 0) {
            ...
            std::cout << "All 5 reverbs are fully functional with 10 parameters each!" << std::endl;
            std::cout << "Ready for final integration." << std::endl;
        } else {
...
int main() {
    std::cout << "CHIMERA PHOENIX - FINAL REVERB VALIDATION" << std::endl;
    std::cout << "==========================================" << std::endl;
    std::cout << "Testing all 5 professional reverb engines" << std::endl;
    std::cout << "Each with 10 carefully designed parameters\n" << std::endl;
    
    FinalReverbTester tester;
    ...
    // Test ConvolutionReverb
    {
        ...
        tester.testReverb(reverb.get(), "ConvolutionReverb");
    }
