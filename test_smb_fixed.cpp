...
int main() {
    std::cout << "=== Testing SMBPitchShiftFixed ===\n";
    
    const float sampleRate = 44100.0f;
    const int blockSize = 512;
    const float testFreq = 440.0f;
    const float pitchRatio = 1.260f; // 4 semitones up
    ...
    if (error < 5.0f && rms > 0.05f) {
        std::cout << "✓ PASS - SMBPitchShiftFixed works correctly!\n";
        return 0;
    } else {
        std::cout << "✗ FAIL - Not working yet\n";
        return 1;
    }
}