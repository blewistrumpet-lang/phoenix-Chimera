#!/bin/bash
# Build pitch & time effects test

set -e

echo "Building Pitch & Time Effects Test..."
echo "======================================"
...
echo "Compiling pitch_test.cpp..."
clang++ $CPP_FLAGS $INCLUDES $DEFINES \
    -c pitch_test.cpp \
    -o "$OBJ_DIR/pitch_test.o"

echo "Linking pitch test..."
ALL_OBJS=$(find "$OBJ_DIR" -name "*.o" ! -name "standalone_test.o" ! -name "reverb_test.o" ! -name "pitch_test.o" | sort | uniq)

clang++ $CPP_FLAGS \
    "$OBJ_DIR/pitch_test.o" \
    $ALL_OBJS \
    $FRAMEWORKS \
    -L/opt/homebrew/lib -lharfbuzz \
    -o "$BUILD_DIR/pitch_test"

if [ $? -eq 0 ]; then
    echo ""
    echo "✓ Build successful!"
    echo ""
    echo "Run pitch & time analysis with:"
    echo "  cd $BUILD_DIR && ./pitch_test"
else
    echo ""
    echo "✗ Build failed"
    exit 1
