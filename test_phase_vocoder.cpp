...
void testPitchRatio(PhaseVocoderPitchShift& shifter, float ratio, float expectedFreq, const std::string& label) {
    const float sampleRate = 44100.0f;
    const int testSamples = 8192;  // Longer for better frequency resolution
    ...
    // Analyze frequency in stable middle section
    int startIdx = 2000;
    int endIdx = 6000;
...
int main() {
    std::cout << "=== Testing PhaseVocoderPitchShift ===" << std::endl;
    ...
    // Test extreme ratios
    testPitchRatio(shifter, 0.25f, 110.0f, "Two Octaves Down");
    testPitchRatio(shifter, 4.0f, 1760.0f, "Two Octaves Up");
    ...
    std::ofstream file("phase_vocoder_output.csv");
    file << "Sample,Input,Output\n";
    for (int i = 0; i < 500; ++i) {
        file << i << "," << input[i] << "," << output[i] << "\n";
    }
    file.close();
    std::cout << "Saved to phase_vocoder_output.csv" << std::endl;
