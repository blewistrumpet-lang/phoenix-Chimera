...
async def handle_message(request: Dict[str, Any]):
    ...
    try:
        ...
        # CRITICAL FIX: Extract preset from response.data.preset (not response.preset)
        preset_data = response.data.get("preset") if response.data else None
        preset_name = preset_data.get('name', 'Untitled') if preset_data else 'Untitled'

        plugin_response = {
            "success": response.success,
            "type": "preset",  # CRITICAL FIX: Must be "preset" not "response" so plugin routes correctly!
            "message": preset_name,  # Send just the preset name
            "data": {"preset": preset_data},  # CRITICAL FIX: Must wrap in {"preset": ...} for TrinityProtocol
            "metadata": {
                "generation_time": response.metadata.get("generation_time_seconds", 0) if response.metadata else 0,
                "pipeline_version": response.metadata.get("pipeline_version", "3.0") if response.metadata else "3.0",
                "full_message": response.message  # Keep original message in metadata
            }
        }

        logger.info(f"Successfully generated preset '{preset_name}' for prompt")
