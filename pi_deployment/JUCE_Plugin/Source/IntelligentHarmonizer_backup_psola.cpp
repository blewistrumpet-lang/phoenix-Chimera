...
class PSOLAPitchShifter {
    ...
    static constexpr int kMaxPitchMarks = 256;
...
public:
    void init(double sampleRate) {
        ...
        // Initialize buffers
        pitchMarks.reserve(kMaxPitchMarks);
        outputBuffer.resize(kBufferSize, 0.0f);
        
        // Initialize window LUT (Hann)
    ...
    float processPSOLA(const float* input, float* output, int numSamples, float pitchRatio, double sampleRate) noexcept {
        ...
        // Detect pitch period
        float period = pitchDetector.detectPeriod(input, numSamples, sampleRate);
        
        // If we have valid pitch, use PSOLA
        if (period > 20 && period < 2000 && pitchDetector.getConfidence() > 0.6f) {
            currentPeriod = period;
            
            // Find pitch marks (peaks at pitch period intervals)
            detectPitchMarks(input, numSamples);
            
            // Synthesize output using PSOLA
            synthesizePSOLA(output, numSamples);
        } else {
            // No valid pitch - use time-domain granular as fallback
            synthesizeGranular(input, output, numSamples);
        }
...
private:
    void detectPitchMarks(const float* input, int numSamples) {
        if (currentPeriod <= 0) return;
        
        int periodSamples = static_cast<int>(currentPeriod + 0.5f);
        
        // Find peaks at expected pitch period intervals
        for (int pos = lastMarkPos + periodSamples; pos < numSamples; pos += periodSamples) {
            // Find local maximum around expected position
            int searchStart = std::max(0, pos - periodSamples/4);
            int searchEnd = std::min(numSamples-1, pos + periodSamples/4);
            
            int maxPos = pos;
            float maxVal = std::abs(input[pos]);
            
            for (int j = searchStart; j <= searchEnd; ++j) {
                if (std::abs(input[j]) > maxVal) {
                    maxVal = std::abs(input[j]);
                    maxPos = j;
                }
            }
            ...
            // Store pitch mark
            PitchMark mark;
            mark.position = (writePos - numSamples + maxPos + kBufferSize) & kBufferMask;
            mark.period = currentPeriod;
            mark.valid = true;
            
            if (pitchMarks.size() >= kMaxPitchMarks) {
                pitchMarks.erase(pitchMarks.begin());
            }
            pitchMarks.push_back(mark);
            
            lastMarkPos = maxPos;
        }
    }
    
    void synthesizePSOLA(float* output, int numSamples) {
        std::fill(output, output + numSamples, 0.0f);
        
        if (pitchMarks.empty()) return;
        
        // Process each pitch mark
        for (const auto& mark : pitchMarks) {
            if (!mark.valid) continue;
            
            // Calculate grain size (2 * pitch period)
            int grainSize = static_cast<int>(2 * mark.period);
            grainSize = std::min(grainSize, kMaxGrainSize);
            
            // Calculate output position based on pitch ratio
            int outputMarkPos = static_cast<int>(outputPos * currentRatio);
            
            // Extract and window grain centered at pitch mark
            for (int i = -grainSize/2; i < grainSize/2; ++i) {
                int inputIdx = (mark.position + i) & kBufferMask;
                int outputIdx = outputMarkPos + i;
                
                if (outputIdx >= 0 && outputIdx < numSamples) {
                    // Apply window
                    int windowIdx = (i + grainSize/2) * kMaxGrainSize / grainSize;
                    if (windowIdx >= 0 && windowIdx < kMaxGrainSize) {
                        float windowVal = windowLUT[windowIdx];
                        output[outputIdx] += buffer[inputIdx] * windowVal * 0.8f;
                    }
                }
            }
            
            outputPos += static_cast<int>(mark.period / currentRatio);
        }
        
        // Clear old pitch marks
        if (pitchMarks.size() > 10) {
            pitchMarks.erase(pitchMarks.begin(), pitchMarks.begin() + 5);
        }
    }
    
    void synthesizeGranular(const float* input, float* output, int numSamples) {
        // Fallback granular synthesis for non-pitched signals
        const int grainSize = 1024;
        const int grainHop = 256;
        
        std::fill(output, output + numSamples, 0.0f);
        
        for (int pos = 0; pos < numSamples; pos += grainHop) {
            for (int i = 0; i < grainSize && pos + i < numSamples; ++i) {
                int outputIdx = static_cast<int>((pos + i) * currentRatio);
                if (outputIdx >= 0 && outputIdx < numSamples) {
                    float windowVal = windowLUT[i * kMaxGrainSize / grainSize];
                    output[outputIdx] += input[pos + i] * windowVal * 0.5f;
                }
            }
        }
    }
...
struct IntelligentHarmonizer::Impl {
    ...
    void processBlock(float* const* io, int numChannels, int numSamples) noexcept {
        ...
        if (std::abs(intervalValue - 0.5f) < 0.02f) {
            ...
            float rawSemitones = (intervalValue - 0.5f) * 48.0f;
            
            // Find closest musical interval
            int closestIdx = 5;  // Default to unison (index 5 = 0 semitones)
            float minDist = std::abs(rawSemitones);
            
            for (int i = 0; i < 12; ++i) {
                float dist = std::abs(rawSemitones - kMusicalIntervals[i]);
                if (dist < minDist) {
                    minDist = dist;
                    closestIdx = i;
                }
            ...
            // Smooth quantization for musical feel
            float targetInterval = kMusicalIntervals[closestIdx];
            float smoothing = 1.0f - std::min(1.0f, minDist / 3.0f);
            semitones = rawSemitones * (1.0f - smoothing * 0.7f) + targetInterval * smoothing * 0.7f;
        }
        ...
        for (int ch = 0; ch < numChannels; ++ch) {
            ...
            for (int voice = 0; voice < activeVoices; ++voice) {
                ...
                // Process with TRUE PSOLA
                float detectedPeriod = shifter.processPSOLA(
                    dryBuffer.data(), 
                    voiceBuffer.data(), 
                    numSamples,
                    pitchRatio,
                ...
                if (formantValue > 0.01f && detectedPeriod > 0) {
                    for (int i = 0; i < numSamples; ++i) {
