...
std::map<int, float> getDefaultParameters(int engineId) {
    ...
    switch (engineId) {
        ...
        case ENGINE_CHAOS_GENERATOR: // Chaos Generator
            defaults[0] = 0.3f;   // Rate - Moderate chaos rate for audible effect
            defaults[1] = 0.5f;   // Depth - Substantial chaos depth for drama
            defaults[2] = 0.0f;   // Type - Lorenz attractor
            defaults[3] = 0.5f;   // Smoothing - Moderate smoothing
            defaults[4] = 4.0f/5.0f; // Target - ModGenerate mode (creates audio from chaos)
            defaults[5] = 0.0f;   // Sync - No tempo sync
            defaults[6] = 0.5f;   // Seed - Random seed
            defaults[7] = 1.0f;   // Mix - Full chaos effect for maximum audibility
            break;
...
int getMixParameterIndex(int engineId) {
    ...
    switch (engineId) {
        ...
        case ENGINE_CHAOS_GENERATOR: return 7; // ChaosGenerator_Platinum - mix parameter at index 7
