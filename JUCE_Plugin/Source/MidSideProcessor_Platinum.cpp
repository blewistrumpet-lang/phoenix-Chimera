...
struct MidSideProcessor_Platinum::Impl {
    ...
    // Mid-Side matrix operations with SIMD
    void encodeMidSide(float left, float right, float& mid, float& side) {
        #if HAS_SSE2
        __m128 lr = _mm_set_ps(0, 0, right, left);
        __m128 sum = _mm_add_ps(lr, _mm_shuffle_ps(lr, lr, _MM_SHUFFLE(2, 3, 0, 1)));
        __m128 diff = _mm_sub_ps(lr, _mm_shuffle_ps(lr, lr, _MM_SHUFFLE(2, 3, 0, 1)));
        __m128 scale = _mm_set1_ps(0.7071f); // 1/sqrt(2)
        __m128 result = _mm_mul_ps(_mm_unpacklo_ps(sum, diff), scale);
    ...
    void decodeMidSide(float mid, float side, float& left, float& right) {
        #if HAS_SSE2
        __m128 ms = _mm_set_ps(0, 0, side, mid);
        __m128 scale = _mm_set1_ps(0.7071f);
        ms = _mm_mul_ps(ms, scale);
        __m128 sum = _mm_add_ps(ms, _mm_shuffle_ps(ms, ms, _MM_SHUFFLE(2, 3, 0, 1)));
        __m128 diff = _mm_sub_ps(ms, _mm_shuffle_ps(ms, ms, _MM_SHUFFLE(2, 3, 0, 1)));
        
        alignas(16) float res[4];
        _mm_store_ps(res, _mm_unpacklo_ps(sum, diff));
        left = res[0];
        right = res[1];
        #else
        // Scalar fallback
...
void MidSideProcessor_Platinum::process(juce::AudioBuffer<float>& buffer) {
    ...
    for (int sample = 0; sample < numSamples; ++sample) {
        ...
        FLUSH_DENORMAL(leftChannel[sample]);
        FLUSH_DENORMAL(rightChannel[sample]);
    }
}
