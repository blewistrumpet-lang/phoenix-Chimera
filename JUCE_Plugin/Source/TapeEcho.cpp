...
float TapeEcho::processSample(float input, int channel) {
    ...
    // Write to delay line
    state.delayLine.write(recordSignal + feedbackSignal);
...
float TapeEcho::DelayLine::readInterpolated(float delaySamples) const {
    ...
    int pos0 = static_cast<int>(readPos) % m_size;
    int pos1 = (pos0 + 1) % m_size;
    int pos2 = (pos0 + 2) % m_size;
    int pos3 = (pos0 + 3) % m_size;
