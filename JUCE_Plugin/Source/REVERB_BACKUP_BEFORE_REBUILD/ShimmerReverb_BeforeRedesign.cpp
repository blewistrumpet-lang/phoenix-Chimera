...
public:
    ...
    float process(float shimmerAmount, float pitch01) {
        ...
        float baseSemitones = pitch01 * 12.0f; // 0-12 semitones
        float octaveRatio = std::pow(2.0f, baseSemitones / 12.0f); // 1.0 to 2.0
        float fifthRatio = octaveRatio * std::pow(2.0f, 7.0f / 12.0f); // Add perfect fifth
        float secondOctRatio = octaveRatio * 2.0f; // Second octave
        ...
        // Mix harmonics with different gains
        float output = octaveOut * 0.6f +      // Main octave
                      fifthOut * 0.25f +        // Fifth harmonic
                      secondOctOut * 0.15f;     // Second octave
        
        // Apply shimmer amount curve
        return output * shimmerAmount * shimmerAmount * 0.8f;
    }
...
void ShimmerReverb::process(juce::AudioBuffer<float>& buffer) {
    ...
    for (int n=0; n<N; ++n) {
        ...
        // Boost reverb for presence
        const float reverbBoost = 1.5f;
        outL *= reverbBoost;
        outR *= reverbBoost;
        ...
        if (shAmt > 0.01f) {
            ...
            // Feed to harmonic processor
            s_harmonicShimmer.push(fdnSum);
            float shimmerOut = s_harmonicShimmer.process(shAmt, pitch01);
            
            // Mix shimmer with stereo width variation
            outL += shimmerOut * (1.0f + 0.2f * width01);
            outR += shimmerOut * (1.0f - 0.2f * width01);
        }
