...
// Proper Signalsmith pitch shifter for shimmer
class SignalsmithShimmer {
public:
    SignalsmithShimmer() {
    ...
    void prepare(double sampleRate, int maxBlockSize) {
        ...
        // Configure for real-time pitch shifting
        stretcher->presetDefault(1, (float)sampleRate);
        
        // Set up for octave shifting (2x frequency)
        stretcher->setTransposeFactor(2.0f); // Octave up
        
        // We need buffering for the stretcher
        ...
        // Create input/output pointer arrays for Signalsmith
        inputPtrs.resize(1);
        outputPtrs.resize(1);
    ...
    void setPitchShift(float semitones) {
        // Clamp to reasonable range for shimmer (12-24 semitones)
        semitones = std::max(12.0f, std::min(24.0f, semitones));
    ...
    void processBlock(const float* input, float* output, int numSamples, float shimmerAmount) {
        ...
        // For pitch shift up with no time stretch, we read more samples than we output
        // But Signalsmith handles this internally
        ...
        // Process with same number of input and output samples
        // Signalsmith will handle the pitch shifting internally
        stretcher->process(inputPtrs.data(), numSamples, outputPtrs.data(), numSamples);
        
        // Apply shimmer amount
        for (int i = 0; i < numSamples; ++i) {
            output[i] *= shimmerAmount * shimmerAmount; // Squared for smoother curve
        }
    }
...
private:
    ...
    // Pointer arrays for Signalsmith API
...
void ShimmerReverb::process(juce::AudioBuffer<float>& buffer) {
    ...
    for (int n = 0; n < N; ++n) {
        ...
        // Collect signal for shimmer processing
        shimmerBuffer.setSample(0, n, 0.5f * (a + b + c + d));

        // Width processing on reverb
        const float widthScaled = width01 * width01;
        stereoWidth(outL, outR, widthScaled);

        // Mix dry/wet (without shimmer for now)
        float yL, yR;
        if (mix01 < 0.001f) {
            yL = dryL;
            yR = dryR;
    ...
    // Process shimmer as a block if enabled
    if (shAmt > 0.01f) {
        // Set pitch shift amount (12-24 semitones based on pitch parameter)
        g_signalsmithShimmer.setPitchShift(12.0f + pitch01 * 12.0f);
        
        // Process the collected reverb signal through Signalsmith
        ...
        // Mix shimmer back into output with stereo width
        for (int n = 0; n < N; ++n) {
            float shimmerSample = shimmerOutput[n] * 2.0f; // Boost shimmer level
            
            // Add shimmer with slight stereo spread
            Lp[n] += shimmerSample * (1.0f + 0.1f * width01);
            if (Rp) Rp[n] += shimmerSample * (1.0f - 0.1f * width01);
        }
    }
