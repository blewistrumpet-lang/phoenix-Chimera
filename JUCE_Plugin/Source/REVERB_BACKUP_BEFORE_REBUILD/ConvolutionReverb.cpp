...
public:
    ...
    float processConvolution(float input, int channel, int sampleIndex) {
        // Select the appropriate buffer
        auto& buffer = (channel == 0) ? convBufferL : convBufferR;
        
        // Calculate write position for this sample
        int writePos = (convWritePos + sampleIndex) % static_cast<int>(buffer.size());
        ...
        for (int i = 0; i < irSize; i++) {
            int readPos = (writePos - i + static_cast<int>(buffer.size())) % static_cast<int>(buffer.size());
    ...
    void process(juce::AudioBuffer<float>& buffer) {
        ...
        for (int sample = 0; sample < numSamples; sample++) {
            ...
            // Process convolution
            float wetL = processConvolution(inputL, 0, sample);
            float wetR = processConvolution(inputR, 1, sample);
        ...
        // Update convolution write position after processing all samples
