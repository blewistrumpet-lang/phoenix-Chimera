...
void ShimmerReverb::prepareToPlay(double sampleRate, int samplesPerBlock) {
    ...
    // setup 4-delay FDN
    L_[0].delay.allocate(static_cast<int>(0.030 * sr));
    L_[1].delay.allocate(static_cast<int>(0.034 * sr));
    L_[2].delay.allocate(static_cast<int>(0.039 * sr));
    L_[3].delay.allocate(static_cast<int>(0.041 * sr));

    // pre-delay & shimmer
    preDelay_.allocate(static_cast<int>(0.250 * sr));
    shimmer_.prepare(sr);
...
void ShimmerReverb::process(juce::AudioBuffer<float>& buffer) {
    ...
    for (int n=0; n<N; ++n) {
        ...
        // === ARCHITECTURAL FIX 3: Proper input gating ===
        // Gate input to reverb based on mix (but keep tails running)
        ...
        // === ARCHITECTURAL FIX 4: Gate by mix for efficiency ===
        // Only feed new signal to reverb when mix > 0
        // This prevents coloring the dry signal but allows tails to ring
        reverbInput *= mix01;
        ...
        const float normFactor = 1.0f / 1.2f; // Sum of absolute coefficients
        outL *= normFactor;
        outR *= normFactor;
        ...
        // Simple crossfade without extra gain stages
        float yL, yR;
        if (mix01 < 0.001f) {
            // Pure dry signal when mix is essentially 0
            yL = dryL;
            yR = dryR;
        } else {
            // Crossfade between dry and wet
            // Note: wet signal is already gated by mix at input
            // So we need to compensate here for consistent level
            float wetGain = (mix01 > 0.1f) ? 1.0f : (1.0f / std::max(0.1f, mix01));
            wetGain = std::min(2.0f, wetGain); // Limit compensation
            
            yL = dryL * (1.0f - mix01) + outL * mix01 * wetGain * 0.7f;
            yR = dryR * (1.0f - mix01) + outR * mix01 * wetGain * 0.7f;
        }
