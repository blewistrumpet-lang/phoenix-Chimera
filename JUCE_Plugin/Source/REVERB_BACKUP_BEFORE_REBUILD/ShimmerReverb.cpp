...
void ShimmerReverb::updateParameters(const std::map<int, float>& params) {
    // Debug output
    std::cout << "ShimmerReverb::updateParameters called with " << params.size() << " params" << std::endl;
    
    for (const auto& [index, value] : params) {
        std::cout << "  Setting param " << index << " = " << value << std::endl;
        if (index < 5) {
            pImpl->setParameter(index, value);
            std::cout << "    After setParameter, mix = " << pImpl->mix << std::endl;
        }
    }
    
    // Verify the mix value was set
    std::cout << "  Final mix value = " << pImpl->mix << std::endl;
}
