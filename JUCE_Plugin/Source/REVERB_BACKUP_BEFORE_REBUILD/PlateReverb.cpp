...
public:
    ...
    void updateCoefficients() {
        ...
        const float baseDelayMs[NUM_DELAYS] = {37.0f, 41.0f, 43.0f, 47.0f};
        float sizeScale = 0.3f + sizeParam * 1.4f;  // 0.3x to 1.7x - wider range
        ...
        feedback = 0.5f + sizeParam * 0.45f;  // 0.5 to 0.95 - wider range
        
        // Damping coefficient - FIXED: higher value = MORE damping
        dampCoeff = 1.0f - dampingParam * 0.8f;  // 1.0 (no damp) to 0.2 (max damp)
    ...
    float applyDamping(float input, float& state) {
        // One-pole lowpass filter - higher dampCoeff = less damping
        state = input * dampCoeff + state * (1.0f - dampCoeff);
        return state;
    }
