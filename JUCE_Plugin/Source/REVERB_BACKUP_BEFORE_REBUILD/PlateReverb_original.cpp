...
public:
    ...
    float read(float delaySamples, PerformanceMode mode) const {
        ...
        // CRITICAL FIX: Proper circular buffer reading
        float readPos = m_writePos - delaySamples - 1;  // -1 because writePos points to next write location
    ...
    float processTap(float input, float delaySamples, PerformanceMode mode) {
        // CRITICAL FIX: Write first, then advance, then read for proper delay
        write(input);
...
struct PlateReverb::ModulatedCombFilter {
    ...
    float process(float input, float feedback, float dampingAmount, PerformanceMode mode, double sampleRate) {
        // Denormal prevention - use smaller value
        input += 1e-15f;
        ...
        // Apply feedback directly without unnecessary scaling
        float fedback = filtered * feedback; // Use full designed feedback for proper tails
        
        // Soft limiting only if necessary
        if (std::abs(fedback) > 0.9f) {
            fedback = 0.9f * std::tanh(fedback / 0.9f); // Unity-gain soft limiter
        }
        
        // Write to delay line - proper gain staging for reverb tails
        float inputScale = 0.7f;  // Higher input gain for proper excitation
        float toWrite = input * inputScale + fedback;
        
        // Only limit at extreme levels to preserve tails
        if (std::abs(toWrite) > 3.0f) {
            toWrite = 3.0f * std::tanh(toWrite / 3.0f);
        }
        
        // Denormal flush - use much lower threshold to preserve tails
        if (std::abs(toWrite) < 1e-15f) toWrite = 0.0f;
...
struct PlateReverb::AllpassFilter {
    ...
    float process(float input, float feedback, PerformanceMode mode) {
        ...
        if (std::abs(toWrite) > 2.0f) {
            toWrite = 2.0f * std::tanh(toWrite / 2.0f);
        }
...
struct PlateReverb::FDN {
    ...
    float process(float input, float feedback, float damping, PerformanceMode mode, double sampleRate) {
        ...
        // Gentle adaptive gain control - only limit at extreme levels
        float energySum = 0.0f;
        for (int i = 0; i < numTaps; ++i) {
            energySum += std::abs(lastOutputs[i]);
        }
        float avgEnergy = energySum / numTaps;
        // Much gentler stability control - only engage at high levels
        float stabilityFactor = 1.0f / (1.0f + std::max(0.0f, avgEnergy - 0.8f) * 0.2f);
        
        for (int i = 0; i < numTaps; ++i) {
            // Proper gain staging for reverb tails
            float inputGain = 0.5f * stabilityFactor;  // Balanced input gain
            float feedbackGain = 0.98f * stabilityFactor; // Near unity for proper tails
            // Direct mixing with adaptive control
            float delayIn = input * inputGain + mixed[i] * feedbackGain;
            
            // Process with full feedback for proper tails
            ...
            // Denormal flush - use much lower threshold to preserve tails
            if (std::abs(lastOutputs[i]) < 1e-15f) lastOutputs[i] = 0.0f;
            
            sum += lastOutputs[i];
        }
        
        // Proper output scaling for natural level
        return sum / (numTaps * 0.9f);  // Less attenuation for better presence
    }
...
PlateReverb::PlateReverb() {
    ...
    m_mix->reset(0.0f);  // Start with dry signal by default
...
void PlateReverb::process(juce::AudioBuffer<float>& buffer) {
    DenormalProtection::DenormalGuard guard; // Enable CPU denormal protection
    if (!m_isInitialized.load()) return;
    ...
    float currentMix = m_mix->getCurrentValue();
    ...
    for (int offset = 0; offset < numSamples; offset += blockSize) {
        ...
        if (numChannels >= 2) {
            ...
            for (int i = 0; i < samplesToProcess; ++i) {
                ...
                // CRITICAL FIX: Don't clamp feedback - use the designed range for proper tails
                // PlateConstants already defines safe limits (0.70 to 0.93)
                ...
                // Get input samples (save for dry mix)
                float dryL = leftData[i];
                float dryR = rightData[i];
                
                // CRITICAL FIX: Always process reverb for proper tails
                // DC blocking for reverb path
                float inputL = m_dcBlockers[0]->process(dryL);
                float inputR = m_dcBlockers[1]->process(dryR);
                
                // Input limiting for reverb path
                ...
                // Apply mix with original dry signal
                leftData[i] = dryL * (1.0f - mix) + reverbL * mix;
                rightData[i] = dryR * (1.0f - mix) + reverbR * mix;
            }
...
std::pair<double, double> PlateReverb::processReverbSample(double inputL, double inputR) {
    ...
    // Mix to mono for reverb input with proper gain staging
    float monoInput = (inputL + inputR) * 0.5f; // Proper stereo-to-mono mix
    
    // Early reflections
    auto [earlyL, earlyR] = m_earlyReflections->process(monoInput, perfMode);
    
    // Input diffusion with controlled feedback
    float diffused = monoInput;
    for (auto& diffuser : m_inputDiffusion) {
        diffused = diffuser->process(diffused, 0.65f, perfMode); // Moderate diffusion feedback
    }
    ...
    float fdnR = m_fdnRight->process(diffused * 0.98f, feedback, damping, perfMode, m_sampleRate);
    
    // Combine early and late reflections with proper gain
    ...
    // Soft limiting with unity gain
    const float threshold = 0.95f;
    if (std::abs(outL) > threshold) {
        outL = threshold * std::tanh(outL / threshold);
    }
    if (std::abs(outR) > threshold) {
        outR = threshold * std::tanh(outR / threshold);
    }
