...
void SpringReverb_Platinum::process(juce::AudioBuffer<float>& buffer) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int k=0;k<kLines;++k) {
            // CRITICAL FIX: Prevent cross-channel feedback accumulation
            float crossL = loopGain * (0.6f * L_[(k+0)%kLines].lastOut + 0.4f * R_[(k+1)%kLines].lastOut);
            float crossR = loopGain * (0.6f * R_[(k+0)%kLines].lastOut + 0.4f * L_[(k+1)%kLines].lastOut);
            
            // Soft limit the feedback before adding to input
            if (std::abs(crossL) > 0.9f) crossL = 0.9f * std::tanh(crossL / 0.9f);
            if (std::abs(crossR) > 0.9f) crossR = 0.9f * std::tanh(crossR / 0.9f);
            
            float fbinL = inL + crossL;
            float fbinR = inR + crossR;
            
            // Extra safety on total
            if (std::abs(fbinL) > 0.95f) fbinL = 0.95f * std::tanh(fbinL / 0.95f);
            if (std::abs(fbinR) > 0.95f) fbinR = 0.95f * std::tanh(fbinR / 0.95f);
