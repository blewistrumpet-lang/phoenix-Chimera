// IntelligentHarmonizer.cpp - CORRECTLY IMPLEMENTED TRUE TD-PSOLA
// Time-Domain Pitch Synchronous Overlap-Add with proper synthesis mark calculation
//
// Key insight: Synthesis marks are spaced by α * period where α = 1/pitchRatio
// - To shift UP (pitchRatio > 1): α < 1, marks CLOSER together  
// - To shift DOWN (pitchRatio < 1): α > 1, marks FURTHER apart
...
// ==================== CORRECT TD-PSOLA Pitch Shifter ====================
class CorrectPSOLA {
    static constexpr int kBufferSize = 131072;  // Large buffer for history
    ...
    // Pitch marks
    struct PitchMark {
        int position;      // Position in buffer (samples)
        float period;      // Local pitch period
    };
    
    std::deque<PitchMark> analysisPitchMarks;
    
    // Synthesis state
    float nextSynthesisPosition{0.0f};  // Next position to place a grain
    int lastAnalysisMarkUsed{0};        // Index of last analysis mark used
...
public:
    void init(double sampleRate) {
        ...
        // Create Hann window (max 4096 samples = ~85ms at 48kHz)
    ...
    void process(const float* input, float* output, int numSamples, float pitchRatio) noexcept {
        ...
        if (detectedPeriod > 20 && detectedPeriod < 1000 && pitchDetector.getConfidence() > 0.3f) {
            // Smooth the period to avoid jumps
            if (smoothedPeriod == 0.0f) {
                smoothedPeriod = detectedPeriod;
            } else {
                smoothedPeriod = 0.9f * smoothedPeriod + 0.1f * detectedPeriod;
            }
            currentPeriod = smoothedPeriod;
            
            // Step 2: Find analysis pitch marks (simplified: every period samples)
            updateAnalysisPitchMarks(numSamples);
            
            // Step 3: Perform PSOLA synthesis
            synthesizePSOLA(output, numSamples, pitchRatio);
            
        } else {
            // No valid pitch - simple resampling fallback
            simplePitchShift(input, output, numSamples, pitchRatio);
        }
    }
    
private:
    void updateAnalysisPitchMarks(int numSamples) {
        if (currentPeriod <= 0) return;
        
        int periodSamples = static_cast<int>(currentPeriod + 0.5f);
        
        // Add new pitch marks at period intervals
        int lastMarkPos = analysisPitchMarks.empty() ? 0 : 
                         (analysisPitchMarks.back().position - inputWritePos + numSamples + kBufferSize) & kBufferMask;
        
        while (lastMarkPos + periodSamples < numSamples) {
            lastMarkPos += periodSamples;
            
            PitchMark mark;
            mark.position = (inputWritePos - numSamples + lastMarkPos + kBufferSize) & kBufferMask;
            mark.period = currentPeriod;
            
            analysisPitchMarks.push_back(mark);
            
            // Keep deque size reasonable
            while (analysisPitchMarks.size() > 100) {
                analysisPitchMarks.pop_front();
            }
        }
    }
    
    void synthesizePSOLA(float* output, int numSamples, float pitchRatio) {
        if (analysisPitchMarks.empty()) return;
        
        // CRITICAL: α = 1/pitchRatio for synthesis mark spacing
        // For octave up (pitchRatio=2): α=0.5, marks closer
        // For octave down (pitchRatio=0.5): α=2, marks further
        float alpha = 1.0f / pitchRatio;
        
        // Synthesis loop: place grains at synthesis positions
        while (nextSynthesisPosition < numSamples) {
            // Find nearest analysis mark
            if (lastAnalysisMarkUsed >= analysisPitchMarks.size()) {
                lastAnalysisMarkUsed = 0;  // Wrap around
            }
            
            if (lastAnalysisMarkUsed < analysisPitchMarks.size()) {
                const PitchMark& analysisMark = analysisPitchMarks[lastAnalysisMarkUsed];
                
                // Window size = 2 * period
                int windowSize = static_cast<int>(2 * analysisMark.period);
                windowSize = std::min(windowSize, 4096);  // Limit to window cache size
                
                // Extract and window grain centered at analysis mark
                int halfWindow = windowSize / 2;
                
                for (int i = -halfWindow; i < halfWindow; ++i) {
                    // Source position in input buffer
                    int srcPos = (analysisMark.position + i) & kBufferMask;
                    
                    // Destination position in output
                    int dstPos = static_cast<int>(nextSynthesisPosition) + i;
                    
                    if (dstPos >= 0 && dstPos < numSamples) {
                        // Get window value
                        int winIdx = i + halfWindow;
                        if (winIdx >= 0 && winIdx < windowSize) {
                            float windowVal = hannWindow[winIdx * 4096 / windowSize];
                            
                            // Overlap-add with proper gain compensation
                            output[dstPos] += inputBuffer[srcPos] * windowVal * 0.7f;
                        }
                    }
                }
                
                // Move to next analysis mark
                lastAnalysisMarkUsed++;
            }
            
            // CRITICAL: Advance synthesis position by α * period
            nextSynthesisPosition += alpha * currentPeriod;
        }
        
        // Update for next block
        nextSynthesisPosition -= numSamples;
        if (nextSynthesisPosition < 0) nextSynthesisPosition = 0;
    }
    
    void simplePitchShift(const float* input, float* output, int numSamples, float pitchRatio) {
        // Simple resampling for non-pitched signals
        ...
        for (int i = 0; i < numSamples; ++i) {
            ...
            if (readPosInt < numSamples - 1) {
                float sample = input[readPosInt] * (1.0f - frac) + 
                              input[readPosInt + 1] * frac;
...
struct IntelligentHarmonizer::Impl {
    ...
    struct ChannelState {
        ...
        std::array<CorrectPSOLA, kMaxVoices> pitchShifters;
    ...
    void prepare(double sr, int blockSize) {
        ...
        // Calculate latency
        latencySamples = 256; // Typical PSOLA latency
