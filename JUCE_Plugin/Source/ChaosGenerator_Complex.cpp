...
ChaosGenerator::ChaosGenerator() {
    ...
    m_mix.target = m_mix.current = 0.5f;         // 50% wet/dry mix - IMPORTANT!
}
...
void ChaosGenerator::prepareToPlay(double sampleRate, int samplesPerBlock) {
    ...
    for (auto& channel : m_channelStates) {
        ...
        // Warm up the chaos systems by running them for a bit
        for (int i = 0; i < 100; ++i) {
            channel.lorenz.iterate(0.01, 1.0f, 0.0f);
            channel.rossler.iterate(0.01);
            channel.henon.iterate();
            channel.logistic.iterate();
            channel.ikeda.iterate();
            channel.duffing.iterate(0.01);
        }
    }
}
...
void ChaosGenerator::reset() {
    ...
    for (auto& channel : m_channelStates) {
        ...
        // Warm up the chaos systems after reset
        for (int i = 0; i < 100; ++i) {
            channel.lorenz.iterate(0.01, 1.0f, 0.0f);
            channel.rossler.iterate(0.01);
            channel.henon.iterate();
...
void ChaosGenerator::process(juce::AudioBuffer<float>& buffer) {
    ...
    if (++debugCounter % 100 == 0) { // Every 100 blocks
        float inputLevel = buffer.getRMSLevel(0, 0, numSamples);
        if (inputLevel > 0.001f) {
            DBG("ChaosGenerator: Input RMS=" << inputLevel << " Mix=" << m_mix.current << " Depth=" << m_depth.current);
        }
    }
    ...
    if (m_mix.current < 0.001f) {
        // Completely dry - no processing needed
        return;
    }
    ...
    for (int channel = 0; channel < numChannels; ++channel) {
        ...
        for (int sample = 0; sample < numSamples; ++sample) {
            ...
            if (++state.sampleCounter >= state.updateInterval) {
                ...
                // Scale chaos output more dramatically based on depth
                state.chaosValue.setTarget(chaosOutput * depth * 2.0f); // Double the effect
            }
...
float ChaosGenerator::applyModulation(float input, float chaos, ModTarget target, ChannelState& state) {
    ...
    switch (target) {
        case AMPLITUDE: {
            // Amplitude modulation - make it more dramatic
            float gain = 1.0f + chaos * 2.0f; // Can go from 0 to 3x gain
            gain = std::max(0.0f, gain); // Ensure non-negative
            output = input * gain;
            break;
        }
