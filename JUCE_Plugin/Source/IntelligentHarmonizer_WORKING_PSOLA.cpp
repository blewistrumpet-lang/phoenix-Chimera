...
public:
    ...
    void process(const float* input, float* output, int numSamples, float pitchRatio) noexcept {
        ...
        if (detectedPeriod > 30 && detectedPeriod < 800) {
            // Smooth period changes
            currentPeriod = 0.9f * currentPeriod + 0.1f * detectedPeriod;
        }
        ...
        // PSOLA synthesis
        if (currentPeriod > 0) {
            // Calculate how much to advance input vs output phases
            float inputAdvance = 1.0f;
            float outputAdvance = pitchRatio;
            
            // Grain size is 2 * period
            int grainSize = static_cast<int>(2 * currentPeriod);
            grainSize = std::min(grainSize, kMaxGrainSize);
            
            // Process using overlap-add
            while (outputPhase < numSamples) {
                // Find the input position for this output position
                int inputPos = static_cast<int>(inputPhase);
                int outputPos = static_cast<int>(outputPhase);
                
                // Extract and window a grain from input history
                for (int i = -grainSize/2; i < grainSize/2; ++i) {
                    // Calculate positions
                    int srcSample = inputPos + i;
                    int dstSample = outputPos + i;
                    
                    // Check bounds
                    if (dstSample >= 0 && dstSample < numSamples) {
                        // Map to history buffer
                        int historyIdx = (historyWritePos - numSamples + srcSample + kHistorySize) % kHistorySize;
                        ...
                        if (winIdx >= 0 && winIdx < grainSize) {
                            ...
                            // Overlap-add
                            output[dstSample] += inputHistory[historyIdx] * windowVal * 0.5f;
                        }
                    }
                }
                
                // Advance phases
                // Key insight: we advance through input and output at different rates
                inputPhase += currentPeriod;
                outputPhase += currentPeriod * pitchRatio;
            }
            
            // Update phases for next block
            inputPhase -= numSamples;
            outputPhase -= numSamples;
            
            // Keep phases positive
            if (inputPhase < 0) inputPhase = 0;
            if (outputPhase < 0) outputPhase = 0;
            
        } else {
            // No valid pitch - use simple resampling
            float readPos = 0.0f;
            ...
            for (int i = 0; i < numSamples; ++i) {
                ...
                if (readPosInt < numSamples - 1) {
                    output[i] = input[readPosInt] * (1.0f - frac) + 
