...
private:
    ...
    struct SpringWaveguide {
        ...
        float process(float input, float feedback, float aging, double sampleRate) {
            ...
            float springModulation = modulation.process(1.0f, sampleRate);
            float modulatedDelay = size + springModulation * size * 0.01f;
            
            // Ensure modulated delay is within bounds
            while (modulatedDelay < 1.0f) modulatedDelay += delayLine.size();
            while (modulatedDelay >= delayLine.size()) modulatedDelay -= delayLine.size();
            
            // Hermite interpolation for high-quality delay reading
            float readPos = writePos - modulatedDelay;
            while (readPos < 0) readPos += delayLine.size();
            
            int idx0 = static_cast<int>(readPos);
            int idx1 = (idx0 + 1) % delayLine.size();
            int idx2 = (idx0 + 2) % delayLine.size();
            int idx3 = (idx0 + 3) % delayLine.size();
            ...
            // Process through dispersion filters (simulates spring frequency dependency)
            ...
            // Bidirectional wave propagation modeling
            float forward = delayed;
            float backward = auxiliaryLine[writePos];
            auxiliaryLine[writePos] = forward * 0.1f; // Small amount for realistic reflections
            
            // Write to main delay line with feedback and spring nonlinearities
            float nonlinearFeedback = feedback * (1.0f + aging * 0.2f);
            if (std::abs(delayed) > 0.3f) {
                // Spring saturation modeling
                delayed = std::tanh(delayed * 1.5f) * 0.7f;
            }
            
            delayLine[writePos] = input + delayed * nonlinearFeedback + backward;
            writePos = (writePos + 1) % delayLine.size();
            
            return delayed + backward * 0.3f; // Mix direct and reflected waves
        }
    ...
    struct DripGenerator {
        ...
        float process(float input, float amount) {
            ...
            // Generate drip on transients
            if (diff > 0.1f && dist(rng) > threshold) {
                float drip = (dist(rng) - 0.5f) * 2.0f;
                drip *= std::exp(-diff * 10.0f); // Decay quickly
                return drip * amount;
            }
        ...
        void setThreshold(float amount) {
            threshold = 1.0f - amount * 0.1f; // More drips with higher amount
        }
    };
    ...
    struct ToneControl {
        ...
        float process(float input, float tone) {
            // Simple tilt EQ
            float lowCutoff = 0.05f;
            float highCutoff = 0.3f;
    ...
    struct ChannelState {
        ...
        void prepare(double sampleRate) {
            ...
            // Initialize advanced feedback matrix
            for (int i = 0; i < MAX_SPRINGS; ++i) {
                for (int j = 0; j < MAX_SPRINGS; ++j) {
                    if (i == j) {
                        feedbackMatrix[i][j] = 0.87f; // Self-feedback
                    } else {
                        // Cross-coupling based on spring proximity and characteristics
                        float proximity = 1.0f / (1.0f + std::abs(i - j));
                        feedbackMatrix[i][j] = 0.03f + 0.05f * proximity;
                    }
                }
            }
        }
