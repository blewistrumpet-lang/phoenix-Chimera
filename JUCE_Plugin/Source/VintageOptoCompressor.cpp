    ...
    for (int channel = 0; channel < numChannels; ++channel) {
        ...
        for (int sample = 0; sample < numSamples; ++sample) {
            ...
            if (m_harmonics.current > 0.01f) {
                ...
                input = state.tubeStage.process(input, m_harmonics.current * 0.3f * ageBoost);
            }
            ...
            if (m_harmonics.current > 0.01f) {
                ...
                compressed = state.tubeStage.process(compressed, m_harmonics.current * 0.5f * ageBoost);
            ...
            if (std::abs(output) > 0.95f) {
                output = std::tanh(output * 0.9f) * 1.05f;
            }
...
float VintageOptoCompressor::applyVintageWarmth(float input, float amount) {
    ...
    float driven = input * (1.0f + amount * thermal);
