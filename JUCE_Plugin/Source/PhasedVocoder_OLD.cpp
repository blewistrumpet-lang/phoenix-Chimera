...
void PhasedVocoder::process(juce::AudioBuffer<float>& buffer) {
    ...
    for (int ch = 0; ch < numChannels && ch < pimpl->channelStates.size(); ++ch) {
        ...
        // Fast path for silence - TEMPORARILY DISABLED FOR DEBUGGING
        // The silence detector seems to be too aggressive
        const bool isSilent = false; // state.silenceDetector.process(rms);
...
void PhasedVocoder::Impl::processFrame(ChannelState& state) noexcept {
    ...
    // SIMD-optimized windowing with safe buffer access
#ifdef __AVX2__
    for (size_t i = 0; i < FFT_SIZE; i += 8) {
        // Check if we can safely load 8 consecutive samples
        size_t startIdx = readPosInt + i;
        size_t wrappedStart = wrapIndex(startIdx, state.inputBuffer.size());
        
        if (wrappedStart + 8 <= state.inputBuffer.size()) {
            // Safe to load directly
            __m256 input = _mm256_loadu_ps(&state.inputBuffer[wrappedStart]);
            __m256 win = _mm256_load_ps(&state.window[i]);
            __m256 result = _mm256_mul_ps(input, win);
            _mm256_store_ps(&state.grainBuffer[i], result);
        } else {
            // Load samples individually to handle wraparound
            for (size_t j = 0; j < 8; ++j) {
                size_t idx = wrapIndex(startIdx + j, state.inputBuffer.size());
                state.grainBuffer[i + j] = state.inputBuffer[idx] * state.window[i + j];
            }
        }
    }
#elif defined(__SSE2__)
    for (size_t i = 0; i < FFT_SIZE; i += 4) {
        // Check if we can safely load 4 consecutive samples
        size_t startIdx = readPosInt + i;
        size_t wrappedStart = wrapIndex(startIdx, state.inputBuffer.size());
        
        if (wrappedStart + 4 <= state.inputBuffer.size()) {
            // Safe to load directly
            __m128 input = _mm_loadu_ps(&state.inputBuffer[wrappedStart]);
...
void PhasedVocoder::Impl::synthesizeFrame(ChannelState& state) noexcept {
    ...
    // CRITICAL FIX: The scaling was way too small (0.000325)
    // For JUCE's normalized FFT and Hann window with 75% overlap
    // We need much more aggressive scaling to restore proper levels
    const float scale = 2.0f / 3.0f;  // Empirically determined for proper output level
    
#ifdef __AVX2__
    const __m256 vScale = _mm256_set1_ps(scale);
    for (size_t i = 0; i < FFT_SIZE; i += 8) {
        size_t startIdx = state.outputWritePos + i;
        size_t wrappedStart = wrapIndex(startIdx, state.outputBuffer.size());
        
        // Check if we can safely load/store 8 consecutive samples
        if (wrappedStart + 8 <= state.outputBuffer.size()) {
            // Safe to use SIMD
            ...
            __m256 existing = _mm256_loadu_ps(&state.outputBuffer[wrappedStart]);
            _mm256_storeu_ps(&state.outputBuffer[wrappedStart], _mm256_add_ps(existing, result));
        } else {
            // Handle wraparound sample by sample
            for (size_t j = 0; j < 8; ++j) {
                size_t idx = wrapIndex(startIdx + j, state.outputBuffer.size());
                float sample = state.fftBuffer[i + j].real() * state.window[i + j] * scale;
                state.outputBuffer[idx] += sample;
            }
        }
    }
#elif defined(__SSE2__)
    ...
    for (size_t i = 0; i < FFT_SIZE; i += 4) {
        ...
        // Check if we can safely load/store 4 consecutive samples
        if (wrappedStart + 4 <= state.outputBuffer.size()) {
            // Safe to use SIMD
            ...
            __m128 existing = _mm_loadu_ps(&state.outputBuffer[wrappedStart]);
            _mm_storeu_ps(&state.outputBuffer[wrappedStart], _mm_add_ps(existing, result));
        } else {
            // Handle wraparound sample by sample
            for (size_t j = 0; j < 4; ++j) {
                size_t idx = wrapIndex(startIdx + j, state.outputBuffer.size());
