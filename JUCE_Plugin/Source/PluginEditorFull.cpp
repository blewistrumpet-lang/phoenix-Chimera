                        ...
                        if (slots.isArray())
                        {
                            for (int i = 0; i < juce::jmin(6, slots.size()); ++i)
                            {
                                ...
                                if (slot.hasProperty("engine_id"))
                                {
                                    ...
                                    // Convert engine ID to choice index and set
                                    int choiceIndex = ChimeraAudioProcessor::engineIDToChoiceIndex(engineId);
                                    engineSelectors[i].setSelectedId(choiceIndex + 1);
                                }
                                ...
                                if (slot.hasProperty("parameters"))
                                {
                                    ...
                                    if (params.isArray())
                                    {
                                        for (int j = 0; j < juce::jmin(PARAMS_PER_SLOT, params.size()); ++j)
                                        {
                                            float value = params[j];
                                            slotParamSliders[i][j].setValue(value);
                                        }
                                    }
                                }
                            }
